...
def test_baseline_fails_when_training_target_has_one_class(tmp_path: Path) -> None:
    ...
    frames = {
        "train": pd.DataFrame({"patient_id": ["p1", "p2"], "feature_1": [1, 2], "feature_2": [2, 3], "target": [0, 0]}),
        "validation": pd.DataFrame({"patient_id": ["p3", "p4"], "feature_1": [1, 2], "feature_2": [2, 3], "target": [0, 1]}),
        "test": pd.DataFrame({"patient_id": ["p5", "p6"], "feature_1": [1, 2], "feature_2": [2, 3], "target": [0, 1]}),
    }
