...
def filter_input_feature_names(feature_names: list[str]) -> list[str]:
    ...
    if not filtered:
        raise ValueError("No valid LSTM input features remain after filtering protected columns.")

    for feature in filtered:
        if feature in protected:
            raise ValueError(f"Protected column remains in the LSTM feature list: {feature}")
...
def verify_feature_names(feature_names: list[str]) -> None:
    ...
    if any(feature in protected for feature in remaining):
        raise ValueError("Protected columns were not fully removed from the feature list.")
    return None
...
def load_manifest_split_map() -> tuple[dict[str, str], dict[str, set[str]], set[str]]:
    ...
    split_to_patients: dict[str, set[str]] = {split_name: set() for split_name in SPLIT_NAMES}
    ...
    for row in manifest.itertuples(index=False):
        ...
        if patient_id in patient_to_split and patient_to_split[patient_id] != split_name:
...
def iter_patient_blocks(split_name: str, feature_names: list[str]):
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"Missing processed CSV for split '{split_name}': {input_path}")
    ...
    for chunk in pd.read_csv(input_path, chunksize=100_000):
        ...
        if missing_columns:
            raise ValueError(f"{split_name}: missing required columns in processed data -> {missing_columns}")
        ...
        for row in chunk.itertuples(index=False, name=None):
            ...
            if patient_id != current_patient_id:
                yield pd.DataFrame(current_rows, columns=["patient_id", "ICULOS", "target", *feature_names])
                current_patient_id = patient_id
                current_rows = [row]
            else:
...
def patient_sequence_counts(patient_df: pd.DataFrame, feature_names: list[str]) -> tuple[int, int, int, int, int]:
    ...
    return valid_sequences, positive_sequences, negative_sequences, candidate_windows, skipped_windows


def count_split_sequences(split_name: str, feature_names: list[str]) -> dict[str, int]:
    summary = {"candidate_windows": 0, "skipped_windows": 0, "sequences": 0, "positive": 0, "negative": 0}

    for patient_block in iter_patient_blocks(split_name, feature_names):
        if patient_block.empty:
            continue
        sequences, positive, negative, candidate_windows, skipped_windows = patient_sequence_counts(patient_block, feature_names)
...
def fill_split_sequences(split_name: str, feature_names: list[str], total_sequences: int) -> dict[str, int]:
    num_features = len(feature_names)
    temp_x_path = SEQUENCE_ROOT / f".{split_name}_X.dat"
    temp_y_path = SEQUENCE_ROOT / f".{split_name}_y.dat"

    X_mem = np.memmap(temp_x_path, dtype=np.float64, mode="w+", shape=(total_sequences, SEQUENCE_LENGTH, num_features))
    ...
    for patient_block in iter_patient_blocks(split_name, feature_names):
        ...
        for start_index in range(len(patient_df) - SEQUENCE_LENGTH + 1):
            ...
            x_values = window_df[feature_names].to_numpy(dtype=np.float64, copy=True)
            if np.isnan(x_values).any() or np.isinf(x_values).any():
                raise ValueError(f"{split_name}: sequence contains NaN or infinite values.")

            target_value = int(window_df["target"].iloc[-1])
            if target_value not in {0, 1}:
                raise ValueError(f"{split_name}: target value must be 0 or 1; got {target_value}.")
    ...
    if write_index != total_sequences:
        raise ValueError(f"{split_name}: expected {total_sequences} sequences, but wrote {write_index}.")
...
def validate_sequence_arrays(X: np.ndarray, y: np.ndarray, feature_count: int, split_name: str) -> None:
    ...
    if X.shape[1] != SEQUENCE_LENGTH:
        raise ValueError(f"{split_name}: expected sequence length {SEQUENCE_LENGTH}, got {X.shape[1]}")
    if X.shape[2] != feature_count:
        raise ValueError(f"{split_name}: feature count mismatch: expected {feature_count}, got {X.shape[2]}")
...
def verify_split_membership(split_name: str, manifest_patient_to_split: dict[str, str], seen_patient_splits: dict[str, str]) -> set[str]:
    expected_patient_ids = {patient_id for patient_id, split in manifest_patient_to_split.items() if split == split_name}
    ...
    for chunk in pd.read_csv(processed_path, chunksize=100_000):
        ...
        for patient_id in patient_ids:
            if patient_id not in manifest_patient_to_split:
                raise ValueError(f"{split_name}: processed patient {patient_id} is not in the manifest.")
            if manifest_patient_to_split[patient_id] != split_name:
                raise ValueError(f"{split_name}: patient {patient_id} belongs to a different manifest split.")

            if patient_id in seen_patient_splits:
                previous_split = seen_patient_splits[patient_id]
                if previous_split != split_name:
                    raise ValueError(f"Patient {patient_id} appears in multiple processed splits: {previous_split} and {split_name}.")
            else:
    ...
    if observed_patient_ids != expected_patient_ids:
        ...
        raise ValueError(f"{split_name}: processed patient IDs do not match manifest assignment. Missing={missing[:10]}, Extra={extra[:10]}")

    return observed_patient_ids


def validate_global_coverage(manifest_patient_to_split: dict[str, str], observed_by_split: dict[str, set[str]]) -> None:
    manifest_patient_ids = set(manifest_patient_to_split)
    all_observed = set().union(*observed_by_split.values()) if observed_by_split else set()
    ...
    if manifest_patient_ids != all_observed:
        ...
        raise ValueError(f"Global coverage mismatch. Missing={missing[:10]}, Extra={extra[:10]}")

    seen: dict[str, str] = {}
    for split_name, patient_ids in observed_by_split.items():
        for patient_id in patient_ids:
            if patient_id in seen and seen[patient_id] != split_name:
                raise ValueError(f"Patient {patient_id} appears in multiple processed splits: {seen[patient_id]} and {split_name}")
            seen[patient_id] = split_name


def generate_sequence_metadata(split_summaries: dict[str, dict[str, int]], feature_names: list[str], source_files: dict[str, str]) -> dict[str, object]:
    return {
        ...
        "positive_sequence_count": int(split_summaries["train"]["positive"] + split_summaries["validation"]["positive"] + split_summaries["test"]["positive"]),
        "negative_sequence_count": int(split_summaries["train"]["negative"] + split_summaries["validation"]["negative"] + split_summaries["test"]["negative"]),
...
def main() -> None:
    ...
    for split_name in SPLIT_NAMES:
        observed_patient_ids = verify_split_membership(split_name, manifest_patient_to_split, seen_patient_splits)
    ...
    for split_name in SPLIT_NAMES:
        ...
        filled_summary = fill_split_sequences(split_name, feature_names, total_sequences)
        split_summaries[split_name] = filled_summary

    source_files = {split_name: str(PROCESSED_ROOT / f"{split_name}.csv") for split_name in SPLIT_NAMES}
    sequence_metadata = generate_sequence_metadata(split_summaries, feature_names, source_files)
