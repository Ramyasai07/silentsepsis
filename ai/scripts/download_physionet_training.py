...
for training_set in TRAINING_SETS:
    ...
    try:
        ...
        print(f"FETCH_ERROR {training_set}: {exc}")
        continue
    ...
    if not psv_links:
        ...
        print(f"NO_PSV_FILES {training_set}")
        continue

    print(f"{training_set}: found {len(psv_links)} .psv files")

    for link in psv_links:
        ...
        try:
            ...
            print(f"FAILED {file_url}: {exc}")

print("\nSUMMARY")
for training_set in TRAINING_SETS:
    count = sum(1 for _ in (raw_root / training_set).glob("*.psv"))
    print(f"{training_set}: {count} files")

failed_count = len(failed_downloads)
print(f"FAILED_DOWNLOADS: {failed_count}")
for item in failed_downloads:
    print(item)

used_bytes = sum(p.stat().st_size for p in raw_root.rglob("*") if p.is_file())
print(f"TOTAL_DISK_SPACE_USED_BYTES: {used_bytes}")
print(f"TOTAL_DISK_SPACE_USED_MB: {used_bytes / (1024 * 1024):.2f} MB")
print(f"RAW_ROOT_EXISTS: {raw_root.exists()}")
print(f"RAW_ROOT_PATH: {raw_root.resolve()}")
print(f"ALL_FILES_WITHIN_AI_RAW: {all(str(p.resolve()).startswith(str(raw_root.resolve())) for p in raw_root.rglob('*') if p.is_file())}")
