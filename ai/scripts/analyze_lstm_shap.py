...
def select_explanation_subset(
    ...
    positive_choice = (
        ...
        else np.random.choice(positive_idx, size=50, replace=False)
    )
    negative_choice = (
        negative_idx
        if len(negative_idx) <= 50
        else np.random.choice(negative_idx, size=50, replace=False)
    )

    positive_count = min(len(positive_choice), 50)
    negative_count = min(len(negative_choice), 50)
    total_samples = min(
        len(positive_choice) + len(negative_choice), EXPLANATION_SAMPLES
    )
...
def validate_shap_values(
    shap_values: np.ndarray, input_shape: tuple[int, int, int], feature_names: list[str]
) -> None:
    ...
    if shap_array.ndim != 3:
        raise ValueError(
            f"SHAP values do not match model input dimensions: expected 3D shape, got {shap_array.shape}"
        )
    if (
        shap_array.shape[0] != input_shape[0]
        or shap_array.shape[1] != input_shape[1]
        or shap_array.shape[2] != input_shape[2]
    ):
        raise ValueError(
            f"SHAP value shape mismatch: expected {(input_shape[0], input_shape[1], input_shape[2])}, got {shap_array.shape}"
        )
    ...
    if len(feature_names) != input_shape[2]:
        raise ValueError(
            f"Feature count mismatch: expected {input_shape[2]}, got {len(feature_names)}"
        )
...
def compute_global_feature_importance(
    ...
    if mean_abs.shape[0] != len(feature_names):
        raise ValueError(
            f"Feature importance length mismatch: expected {len(feature_names)}, got {mean_abs.shape[0]}"
        )
...
def compute_timestep_importance(shap_values: np.ndarray) -> pd.DataFrame:
    ...
    if mean_abs.shape[0] != 12:
        raise ValueError(
            f"Timestep importance length mismatch: expected 12, got {mean_abs.shape[0]}"
        )
...
def main() -> None:
    ...
    if flattened_input_shape != (12, 76):
        raise ValueError(
            f"Unexpected model input shape: {flattened_input_shape}; expected (12, 76)"
        )
    ...
    if shap_array.ndim == 2:
        ...
        shap_array = shap_array.reshape(
            explanation_count, sequence_length, feature_count
        )

    if shap_array.shape != (explanation_count, sequence_length, feature_count):
        raise ValueError(
            f"SHAP values do not match model input dimensions: expected {(explanation_count, sequence_length, feature_count)}, got {shap_array.shape}"
        )
    ...
    if positive_explanation >= 1 and negative_explanation >= 1:
        ...
        positive_subgroup = compute_global_feature_importance(
            shap_array[pos_mask], feature_names
        )
        negative_subgroup = compute_global_feature_importance(
            shap_array[neg_mask], feature_names
        )

    feature_importance.to_csv(
        MODEL_ROOT / "lstm_shap_feature_importance.csv", index=False
    )
    timestep_importance.to_csv(
        MODEL_ROOT / "lstm_shap_timestep_importance.csv", index=False
    )
    ...
    analysis_payload = {
        ...
        "positive_subgroup_importance": (
            positive_subgroup.head(20).to_dict(orient="records")
            if positive_subgroup is not None
            else None
        ),
        "negative_subgroup_importance": (
            negative_subgroup.head(20).to_dict(orient="records")
            if negative_subgroup is not None
            else None
        ),
    }
    ...
    if plt is not None:
        create_feature_importance_plot(
            feature_importance, MODEL_ROOT / "lstm_shap_feature_importance.png"
        )
