...
SPLIT_MANIFEST_PATH = REPO_ROOT / "ai" / "data" / "splits" / "patient_split_manifest.csv"
OUTPUT_PATH = PROCESSED_ROOT / "engineered_patient_features.csv"


def _summarize_features(feature_table: pd.DataFrame) -> dict:
    engineered_columns = [
        col for col in feature_table.columns if any(suffix in col for suffix in [
            "_baseline",
            "_deviation",
            "_pct_deviation",
            "_delta",
    ...
    nan_fraction = feature_table[engineered_columns].isna().mean().mean() if engineered_columns else 0.0
    inf_fraction = np.isinf(feature_table[engineered_columns].select_dtypes(include=["number"])).mean().mean() if engineered_columns else 0.0
    original_columns = [
        col for col in feature_table.columns
        if col not in {"patient_id", "split", "ICULOS", "target"}
        and not any(suffix in col for suffix in [
            "_baseline",
            "_deviation",
            "_pct_deviation",
            "_delta",
    ...
    return {
        ...
        "train_patients": int(feature_table.loc[feature_table["split"] == "train", "patient_id"].nunique()),
        "validation_patients": int(feature_table.loc[feature_table["split"] == "validation", "patient_id"].nunique()),
        "test_patients": int(feature_table.loc[feature_table["split"] == "test", "patient_id"].nunique()),
...
def main() -> None:
    ...
    if feature_table.empty:
        raise ValueError("No processed patient rows were available for feature engineering.")
    ...
    print(f"NaN/Inf fraction in engineered features: {summary['nan_fraction']:.6f} / {summary['inf_fraction']:.6f}")
    print(f"Output path: {OUTPUT_PATH}")
    print("Sample engineered columns:")
    engineered_columns = [column for column in feature_table.columns if any(suffix in column for suffix in ["_baseline", "_deviation", "_pct_deviation", "_delta", "_rolling_mean", "_rolling_std", "shock_index"]) ]
