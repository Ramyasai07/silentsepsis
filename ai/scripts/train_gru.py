#!/usr/bin/env python3
"""Comparable GRU experiment using the same sequence contract as the LSTM pipeline."""
...
def main() -> None:
    ...
    history_path = MODEL_ROOT / "gru_training_history.json"
    ...
    test_df.to_csv(MODEL_ROOT / "gru_test_predictions.csv", index=False)

    print("\nTRAINING COMPLETE")
    print(f"Best validation PR-AUC: {max(history.history['val_pr_auc']):.6f}")
    print("\nVALIDATION RESULTS")
