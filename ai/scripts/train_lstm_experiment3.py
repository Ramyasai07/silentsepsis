#!/usr/bin/env python3
"""Experiment 3: moderated class-weight LSTM baseline based on the existing Experiment 2 behavior."""
...
def compute_moderate_class_weights(y_train: np.ndarray) -> dict[int, float]:
    positive_count, negative_count = compute_class_counts(y_train)
    if positive_count == 0 or negative_count == 0:
        raise ValueError(
            f"Training labels must contain both classes. positive={positive_count}, negative={negative_count}"
        )

    negative_weight = 1.0
    positive_weight = float(np.sqrt(negative_count / positive_count))
...
def main() -> None:
    ...
    print("EXPERIMENT 3: MODERATED CLASS WEIGHT LSTM")
    print("TRAIN:")
    ...
    class_weights = compute_moderate_class_weights(y_train)
    print("\nCLASS WEIGHTS:")
    print(f"  negative: {class_weights[0]}")
    print(f"  positive: {class_weights[1]}")
    print(f"  positive_count: {train_positives}")
    print(f"  negative_count: {train_negatives}")
    ...
    checkpoint_path = MODEL_ROOT / "lstm_experiment3_best.keras"
    ...
    history_path = MODEL_ROOT / "lstm_experiment3_training_history.json"
    ...
    with (MODEL_ROOT / "lstm_experiment3_evaluation.json").open("w", encoding="utf-8") as handle:
    ...
    test_df.to_csv(MODEL_ROOT / "lstm_experiment3_test_predictions.csv", index=False)
