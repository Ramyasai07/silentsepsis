...
def select_explanation_subset(X_test: np.ndarray, y_test: np.ndarray) -> tuple[np.ndarray, np.ndarray, int, int, int]:
    ...
    positive_choice = positive_idx if len(positive_idx) <= 50 else rng.choice(positive_idx, size=50, replace=False)
    negative_choice = negative_idx if len(negative_idx) <= 50 else rng.choice(negative_idx, size=50, replace=False)
...
def get_background_data(train_X: np.ndarray, train_mean: np.ndarray, train_std: np.ndarray) -> np.ndarray:
    ...
    idx = np.random.default_rng(RANDOM_SEED).choice(train_X.shape[0], size=total_samples, replace=False)
...
def validate_shap_shape(shap_array: np.ndarray, expected_shape: tuple[int, int, int], feature_names: list[str]) -> None:
    if shap_array is None:
        raise ValueError("SHAP values are None.")
    if shap_array.shape != expected_shape:
        raise ValueError(f"SHAP values do not match expected dimensions: expected {expected_shape}, got {shap_array.shape}")
    if len(feature_names) != expected_shape[2]:
        raise ValueError(f"Feature count mismatch: expected {expected_shape[2]}, got {len(feature_names)}")
...
def prepare_shap_array(shap_values: object, expected_samples: int, sequence_length: int, feature_count: int) -> np.ndarray:
    ...
    if shap_array.ndim == 2:
        if shap_array.shape != (expected_samples * sequence_length, feature_count):
            raise ValueError(f"Unsupported SHAP output shape: {shap_array.shape}")
        shap_array = shap_array.reshape(expected_samples, sequence_length, feature_count)

    if shap_array.shape != (expected_samples, sequence_length, feature_count):
        raise ValueError(f"SHAP values do not match model input dimensions: expected {(expected_samples, sequence_length, feature_count)}, got {shap_array.shape}")

    return shap_array


def compute_feature_direction(shap_values: np.ndarray, feature_names: list[str]) -> pd.DataFrame:
...
def create_positive_contributor_plot(feature_df: pd.DataFrame, output_path: Path) -> None:
    if plt is None:
        return

    top = feature_df.sort_values("mean_signed_shap", ascending=False).head(20).iloc[::-1]
...
def create_negative_contributor_plot(feature_df: pd.DataFrame, output_path: Path) -> None:
    if plt is None:
        return
...
def main() -> None:
    ...
    flatten_input_shape = tuple(int(value) for value in input_shape[1:]) if isinstance(input_shape, (list, tuple)) and len(input_shape) > 1 else None
    if flatten_input_shape is None:
        raise ValueError("Could not determine the model input shape.")
    if flatten_input_shape != (12, 76):
        raise ValueError(f"Unexpected model input shape: {flatten_input_shape}; expected (12, 76)")
    ...
    shap_array = prepare_shap_array(shap_values, explanation_count, sequence_length, feature_count)
    validate_shap_shape(shap_array, (explanation_count, sequence_length, feature_count), feature_names)

    overall_feature_df = compute_feature_direction(shap_array, feature_names)
    overall_feature_df = overall_feature_df.sort_values("mean_absolute_shap", ascending=False).reset_index(drop=True)
    overall_feature_df.insert(0, "rank_overall", np.arange(1, len(overall_feature_df) + 1))

    positive_rank_df = overall_feature_df.sort_values("mean_signed_shap", ascending=False).reset_index(drop=True)
    positive_rank_df.insert(0, "rank_positive", np.arange(1, len(positive_rank_df) + 1))

    negative_rank_df = overall_feature_df.sort_values("mean_signed_shap", ascending=True).reset_index(drop=True)
    negative_rank_df.insert(0, "rank_negative", np.arange(1, len(negative_rank_df) + 1))

    feature_combined = overall_feature_df.merge(positive_rank_df[["feature", "rank_positive"]], on="feature", how="left")
    feature_combined = feature_combined.merge(negative_rank_df[["feature", "rank_negative"]], on="feature", how="left")
    ...
    if positive_explanation > 0 and negative_explanation > 0:
        ...
        subgroup_analysis["positive"] = compute_feature_direction(shap_array[pos_mask], feature_names).to_dict(orient="records")
        subgroup_analysis["negative"] = compute_feature_direction(shap_array[neg_mask], feature_names).to_dict(orient="records")
    else:
        subgroup_analysis["positive"] = "SKIPPED: insufficient positive or negative explanation samples"
        subgroup_analysis["negative"] = "SKIPPED: insufficient positive or negative explanation samples"

    feature_combined.to_csv(MODEL_ROOT / "lstm_shap_directional_features.csv", index=False)
    timestep_direction.to_csv(MODEL_ROOT / "lstm_shap_directional_timesteps.csv", index=False)
    ...
    with (MODEL_ROOT / "lstm_shap_direction_analysis.json").open("w", encoding="utf-8") as handle:
        json.dump(analysis_payload, handle, indent=2)

    if plt is not None:
        create_positive_contributor_plot(feature_combined, MODEL_ROOT / "lstm_shap_positive_contributors.png")
        create_negative_contributor_plot(feature_combined, MODEL_ROOT / "lstm_shap_negative_contributors.png")
