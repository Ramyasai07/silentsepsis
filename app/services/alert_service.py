...
def evaluate_and_create_alert(db: Session, prediction) -> Alert | None:
    ...
    Duplicate prevention: do not create a new alert if the patient
    already has an open alert (status in active, watching, confirmed).
    """
    trigger_tiers = {"HIGH", "CRITICAL"}
    if prediction.risk_tier not in trigger_tiers:
        return None

    # Compare enum labels case-insensitively in SQL to avoid
    # SQLAlchemy enum-name/value binding differences.
    ...
    message = (
        f"Auto-alert from prediction: tier={prediction.risk_tier} "
    ...
    alert = Alert(
        ...
        severity=severity,
        # Store the enum *value* to avoid SQLAlchemy using enum member names
...
def get_alerts(
    ...
    *,
    ...
    if status is not None:
        # Compare case-insensitively to avoid enum label case mismatches
        # between DB and runtime.
        query = query.where(
            sa.func.lower(sa.cast(Alert.status, sa.Text)) == status.lower()
        )
