...
def get_precision_recall_history(
    db: Session, days: int = 30, bucket_size_days: int = 5
) -> list[dict[str, str | int | None]]:
    """
    Calculate precision/recall history using feedback data.
    THIS IS A FEEDBACK-DERIVED APPROXIMATION, NOT GROUND-TRUTH LABELS.
    Metrics are only as accurate as the clinician feedback provided.
    Precision = (CONFIRMED) / (CONFIRMED + FALSE_POSITIVE)
    Recall = (CONFIRMED) / (CONFIRMED + MISSED_CASE)
    Buckets with insufficient data return null for precision/recall
    ...
    while bucket_start < end_date:
        ...
        buckets.append(
            (bucket_start, bucket_end, f"Day {bucket_num * bucket_size_days + 1}")
        )
    ...
    for bucket_start, bucket_end, bucket_label in buckets:
        stmt = select(
            func.count(
                case((Feedback.feedback_type == FeedbackType.CONFIRMED, 1), else_=None)
            ).label("true_positives"),
            func.count(
                case(
                    (Feedback.feedback_type == FeedbackType.FALSE_POSITIVE, 1),
                    else_=None,
                )
            ).label("false_positives"),
            func.count(
                case(
                    (Feedback.feedback_type == FeedbackType.MISSED_CASE, 1), else_=None
                )
            ).label("false_negatives"),
        ).where(Feedback.created_at >= bucket_start, Feedback.created_at < bucket_end)
        ...
        results.append({"day": bucket_label, "precision": precision, "recall": recall})

    return results


def get_ward_summary_metrics(
    db: Session, ward_id: UUID, days: int = 30
) -> dict[str, object]:
    """
    ...
    latest_preds_subquery = (
        ...
        .join(Prediction.patient)
        .where(Patient.ward_id == ward_id)
    ...
    avg_confirm_stmt = (
        select(
            func.avg(func.extract("epoch", Alert.confirmed_at - Alert.created_at) / 60)
        )
        .select_from(Alert)
        .join(Alert.patient)
        .where(
    ...
    total_patients_stmt = (
        select(func.count()).select_from(Patient).where(Patient.ward_id == ward_id)
    )
...
def get_staff_response_by_ward(
    db: Session, days: int = 30
) -> list[dict[str, str | int]]:
    """
    Calculate staff response rate by ward.
    Reviewed status = any status other than the initial ACTIVE state
    (verified AlertStatus values).
